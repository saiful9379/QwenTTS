...
# Force environment settings for consistency
...
def ensure_left_padding(tokenizer):
    """Ensure tokenizer uses left padding and save configuration"""
    print(f"[DEBUG] Current tokenizer padding_side: {tokenizer.padding_side}")
    print(f"[DEBUG] Current tokenizer pad_token: {tokenizer.pad_token}")
    print(f"[DEBUG] Current tokenizer eos_token: {tokenizer.eos_token}")
    ...
    # Verify settings
    assert tokenizer.padding_side == 'left', f"Failed to set padding_side to 'left', current: {tokenizer.padding_side}"
    print(f"[DEBUG] Verified padding_side: {tokenizer.padding_side}")
    
    # Additional safety check for Qwen models
    if hasattr(tokenizer, 'padding_config'):
        tokenizer.padding_config = {'side': 'left'}
...
class TTSDataset(Dataset):
    def __init__(
        ...
        tokenizer = ensure_left_padding(tokenizer)
    ...
    def pad_sequence(self, sequence, max_length, value=0):
        if len(sequence) >= max_length:
            ...
            # Ensure left padding by putting padding at the beginning
...
# Custom data collator with explicit left padding
class LeftPaddingDataCollator:
    def __init__(self, tokenizer):
        self.tokenizer = tokenizer
        ensure_left_padding(self.tokenizer)
    ...
    def __call__(self, features):
        ...
        # Verify that we have left padding by checking the first few positions
        if 'input_ids' in batch:
            print(f"[DEBUG] Batch input_ids shape: {batch['input_ids'].shape}")
            # Check if padding is on the left (first few positions should be pad tokens)
            if batch['input_ids'].size(0) > 0:
                first_non_pad = (batch['input_ids'][0] != self.tokenizer.pad_token_id).nonzero(as_tuple=True)[0]
                if len(first_non_pad) > 0:
                    first_pos = first_non_pad[0].item()
                    print(f"[DEBUG] First non-pad token at position: {first_pos} (expected > 0 for left padding)")
                else:
                    print(f"[DEBUG] All tokens are pad tokens")
        
        return batch


def save_tokenizer_with_config(tokenizer, path):
    """Save tokenizer with explicit padding configuration"""
    ...
    # Also save explicit configuration file
    config = {
        'padding_side': 'left',
        'pad_token': tokenizer.pad_token,
        'eos_token': tokenizer.eos_token,
    }
    with open(os.path.join(path, 'padding_config.json'), 'w') as f:
        json.dump(config, f, indent=2)
    
    print(f"[DEBUG] Saved tokenizer with left padding config to {path}")


def load_tokenizer_with_config(path, model_name=None):
    """Load tokenizer and ensure correct padding configuration"""
    if os.path.exists(os.path.join(path, 'tokenizer.json')):
        # Load from saved path
        tokenizer = AutoTokenizer.from_pretrained(path)
        print(f"[DEBUG] Loaded tokenizer from {path}")
    else:
        # Load from model name
        tokenizer = AutoTokenizer.from_pretrained(model_name)
        print(f"[DEBUG] Loaded tokenizer from {model_name}")
    
    # Apply configuration
    tokenizer = ensure_left_padding(tokenizer)
    
    # Check if explicit config exists and apply it
    config_path = os.path.join(path, 'padding_config.json')
    if os.path.exists(config_path):
        with open(config_path, 'r') as f:
            config = json.load(f)
        print(f"[DEBUG] Applying explicit padding config: {config}")
...
def main():
    ...
    if last_checkpoint is not None:
        ...
        tokenizer = load_tokenizer_with_config(last_checkpoint)
        print(f"[DEBUG] After loading checkpoint - padding_side: {tokenizer.padding_side}")
        ...
        # Ensure left padding from the start
        tokenizer = ensure_left_padding(tokenizer)
        ...
        # Ensure padding configuration after adding tokens
        tokenizer = ensure_left_padding(tokenizer)
        
        # Save the tokenizer with explicit configuration
        save_tokenizer_with_config(tokenizer, training_args.output_dir)
    ...
    print(f"\n[FINAL CHECK] Tokenizer padding side: {tokenizer.padding_side}")
    print(f"[FINAL CHECK] Tokenizer pad token: {tokenizer.pad_token}")
    assert tokenizer.padding_side == 'left', f"Critical error: padding_side is {tokenizer.padding_side}, expected 'left'"
    ...
    # Use custom data collator that enforces left padding
    ...
    # Additional debug before training
    print(f"\n[PRE-TRAINING CHECK] Trainer tokenizer padding_side: {trainer.tokenizer.padding_side}")
 
    trainer.train(resume_from_checkpoint=last_checkpoint)
 
    # Save final model and tokenizer with explicit configuration
    trainer.save_model(training_args.output_dir)
    save_tokenizer_with_config(tokenizer, training_args.output_dir)
    
    print(f"[FINAL] Saved model and tokenizer with left padding configuration")
